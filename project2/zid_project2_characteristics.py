...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # 需要修改>>>>>> ret_freque_use
    df_daily = ret[ret_freq_use[0]].copy()
    df_daily.index = df_daily.index.to_period('M')
    df_vol_cha = pd.DataFrame()

    for date, group in df_daily.groupby(by='Date'):
        if len(group) > 18:
            vol_df = pd.DataFrame(group.std()).T
            vol_df.index = [date]
            df_vol_cha = pd.concat([df_vol_cha, vol_df], axis=0)

    df_vol_cha.columns = [f"{col}_{cha_name}" for col in df_vol_cha.columns]

    df_vol_cha.dropna(how='all', inplace=True)
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # 感觉还能精简一点 cha_name没用到？！
    df_returns = ret['Monthly'].copy()
    df_merged = pd.merge(df_returns, df_cha, left_index=True, right_index=True, how='left')
    columns_to_shift = df_cha.columns.tolist()
    df_merged[columns_to_shift] = df_merged[columns_to_shift].shift(1)
    df_merged.index = pd.PeriodIndex(df_merged.index, freq='M', name='Year_Month')
