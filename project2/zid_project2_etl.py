...
def read_prc_csv(tic, start, end, prc_col='Adj Close'):
    ...
    tic_data = pd.read_csv(pth, index_col='Date', parse_dates=['Date'])
    sort_date = tic_data.sort_index()
    final_data = sort_date[start:end]
    ser = final_data[prc_col]
    ser.name = tic
    return ser
...
def daily_return_cal(prc):
    ...
    """
    # <COMPLETE THIS PART>
    # daily_return_ser = prc / prc.shift(1) - 1
    daily_return_ser = prc.pct_change()
    daily_return_ser.name = prc.name
    daily_return_ser = daily_return_ser.dropna()
...
def monthly_return_cal(prc):
    ...
    monthly_last = prc.resample('BME').last().dropna()

    # 十八天的这个条件怎么加进去？？？？？
    # series.count?
    monthly_returns = pd.Series(index=monthly_last.index)
    #for i in range(1, len(monthly_last)):
        #current_price = monthly_last.iloc[i]
        #previous_price = monthly_last.iloc[i - 1]
        #monthly_returns.iloc[i] = (current_price - previous_price) / previous_price
    # 直接monthly return = monthly_last / monthly_last.shift(1) - 1/
...
def aj_ret_dict(tickers, start, end):
    ...
    for tic in tickers:
        ticker = tic.lower()
        prc = read_prc_csv(tic=ticker, start=start, end=end)
        daily_return = daily_return_cal(prc)
        monthly_return = monthly_return_cal(prc)
        daily[ticker] = daily_return
        monthly[ticker] = monthly_return

    daily_return_df = pd.DataFrame(daily)
    monthly_return_df = pd.DataFrame(monthly)
    daily_return_df.index.name = 'Date'
    monthly_return_df.index.name = 'Year_Month'

    return_dict = {'Daily': daily_return_df, 'Monthly': monthly_return_df}
    return return_dict
